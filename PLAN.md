...
4. **공통 로더 사용**: 데이터 로드·ID 매핑·아이콘 경로는 `site/shared/` 의 로더를 거친다 (버전 전환·로컬 자산 감지를 한 곳에서 처리).
   - **공통 디자인 계층**: 전역 바·테마는 `shared/scrap.js` 한 줄 로드(`<script src=".../shared/scrap.js" defer>`), 팔레트 토큰은 `shared/scrap.css` 의 `--scrap-*`. 여기를 고치면 전 사이트 반영. 앱별 특화는 각자의 `scrap-theme` 스타일 블록에서 값만 바꾼다. 외부 저장소(모델 뷰어)는 절대 주소로 같은 파일을 로드한다. 테마 변경 시 `scrap-theme` 커스텀 이벤트 발생 — 앱이 후처리 필요하면 구독.
